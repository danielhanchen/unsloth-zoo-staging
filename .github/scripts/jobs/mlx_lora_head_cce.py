...
def _arm(arm, quant, steps, seed, out):
    ...
    json.dump({"arm": arm, "quant": quant, "steps": steps_log, "peak_mb": round(peak, 1),
               "traced": traced, "recompute_budget": budget,
...
def main(argv=None):
    ...
    with C.JobRecorder(a, backend_hint=None) as rec:
        ...
        for arm, d in res.items():
            for s in d["steps"]:
                rec.step(arm=arm, **s)
        rec.summary(arms={k: {kk: v[kk] for kk in ("peak_mb", "traced", "recompute", "recompute_budget",
                                                   "working_set")} | {
        ...
        if "main" not in res:
            return
        base = res["main"]
        rec.check("main_never_traces_lora_cce", base["traced"] == [], base["traced"])
        for arm in ("head", "retain", "recompute"):
            if arm not in res:
                continue
            d = res[arm]
            rec.check(f"{arm}_traces_lora_cce_once", len(d["traced"]) == 1, d["traced"])
