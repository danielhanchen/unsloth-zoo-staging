...
llama.cpp has no bitsandbytes dequantizer. Handing it 4bit weights makes its
converter raise

    NotImplementedError: Quant method is not yet supported: 'bitsandbytes'

but only after reading the entire model, so the user pays a multi-GB download
and a long conversion to reach a message that never mentions 4bit.
Llama3.1_(8B)-Inference fails exactly this way.

The usual route in is save_pretrained_merged(save_method="merged_16bit") on a
model with NO LoRA adapter: merge_and_overwrite_lora only warns and returns
without dequantizing, and 278 notebooks wrap setup in %%capture, which
swallows the warning.

The detector has to find the config wherever it sits, because VLMs keep
theirs under a sub-config -- the same reason _remove_quantization_config in
saving_utils walks nested dicts.
"""
