...
Dataset preparation is farmed out to `dataset_num_proc` fork workers, sized
from CPU count and free RAM. Long-text corpora exceed that budget: on a Colab
T4, CodeForces-cot-Finetune_for_Reasoning_on_CodeForces has the kernel
OOM-kill a worker, and datasets raises

    RuntimeError: One of the subprocesses has abruptly died during map
    operation. To debug the error, disable multiprocessing.

which kills the run inside SFTTrainer.__init__, before any training. The map
is issued deep inside TRL, so the user cannot apply the advice themselves.
