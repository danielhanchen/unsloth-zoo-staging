...
def test_the_logger_is_imported():
    """`logger` was used in this module without being imported. That call sits
    inside a bare `except Exception: pass`, so the NameError was swallowed and
    the LongRoPE warning it belongs to never printed. This patch's warning is
    not inside such a guard, so the import is load-bearing here."""
