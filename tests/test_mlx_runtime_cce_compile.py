...
def test_runtime_cce_zero_tokens_with_non_empty_targets_raises():
    # hidden=0 / targets!=0 must raise instead of silently dropping labels.
    _skip_torch_shim()
...
def test_runtime_cce_int64_wrap_to_ignore_index_poisons_gradients():
    # Wide invalid labels (e.g. 2**32 - 100) narrow to -100 in int32; the
    # backward must propagate NaN from the poisoned lse, not zero-grad.
    _skip_torch_shim()
    from unsloth_zoo.mlx.cce import make_chunked_cross_entropy_loss
...
def test_runtime_cce_int64_invalid_labels_do_not_wrap_to_valid(bad_target):
    # Validate before the int32 narrow so wraparound int64 labels do not
    # masquerade as valid class ids or ignore_index.
    _skip_torch_shim()
    from unsloth_zoo.mlx.cce import make_chunked_cross_entropy_loss
...
def test_runtime_cce_rejects_non_flat_targets():
    # rank-2 / scalar targets would slip past the length check and crash in
...
def test_runtime_cce_invalid_labels_poison_loss_and_gradients(bad_target):
    ...
    # Per-row check: grad_weight's NaN would otherwise hide an lse leak
    # from _poison_invalid_targets into valid rows.
...
def test_compiled_runtime_cce_invalid_labels_poison_loss(bad_target):
    # cover negative and >= vocab_size labels under mx.compile.
    _skip_torch_shim()
    from unsloth_zoo.mlx.cce import make_chunked_cross_entropy_loss
...
def test_compiled_runtime_cce_invalid_labels_poison_gradients(bad_target):
    # NaN must survive aux-lse storage into the VJP under mx.compile.
    _skip_torch_shim()
    from unsloth_zoo.mlx.cce import make_chunked_cross_entropy_loss
