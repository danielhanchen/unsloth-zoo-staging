...
Background, because this is the second design for the same wrapper and the
first one was wrong on live hardware.
...
The subtlety is non-reentrant activation checkpointing. It runs each
checkpointed forward twice -- once to produce activations, once during backward
to recompute them -- and asserts the two saved the same intermediates. A
compiled graph does not save what eager saves, so if the mode changes between
the pack and the recompute, torch aborts:
...
The first design tried to dodge that by NOT latching: retry the compiler every
call, so each pack and its own recompute would agree. rerun16 disproved it on a
live L4 -- Gemma4_(E2B)-Vision hit the same assertion with the non-latching
build demonstrably loaded, logging its new wording one cell earlier. The pack
and the recompute do not run under the same guards, so the compiler can succeed
for one and raise for the other.

So the wrapper latches, which makes every call after the first failure eager
and therefore self-consistent, and `force_eager_fallback()` lets unsloth close
the remaining hole: the single step during which the latch flips.
"""
