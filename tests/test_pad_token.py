...
def test_composite_model_text_vocab_size_is_used_for_pad_range():
    ...
    tokenizer holds 128256 ids and the config declares pad_token_id = 128004.

    Bounding the declared pad by the audio codebook rejected it, so repair fell
    through to synthesising a token and raised
    'Could not find a valid pad token for unsloth/csm-1b'.
    """
    tok = FakeTokenizer(
