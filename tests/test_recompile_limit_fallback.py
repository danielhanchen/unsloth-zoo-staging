...
Gemma4_(E2B)-Vision dies exactly there, inside patch_Gemma4RMSNorm, and by
then Unsloth has already raised the limit from 8 to 1024. Whatever drives
that many recompiles is a separate question; the point here is that running
out of compilation cache is a performance problem, and turning it into a
hard training failure is strictly worse than being slow.
