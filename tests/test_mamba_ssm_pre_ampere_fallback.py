...
mamba_ssm's Triton kernels need Ampere (sm_80)+. On a Tesla T4 (sm_75) the
package imports fine, transformers sets is_fast_path_available = True, and the
layer routes into cuda_kernels_forward -- where Triton then fails to compile
_chunk_state_fwd_kernel with an opaque `RuntimeError: PassManager::run failed`
once training starts. Granite4.0 dies this way on Colab's free T4.

The patch must fire ONLY on pre-Ampere NVIDIA CUDA. ROCm, XPU, MPS and CPU
builds have to be left completely alone, and Ampere+ must keep the fast path.
