...
trace. On Gemma3N audio the hook sits on Gemma3nMultimodalEmbedder inside the
compiled module, and training dies with

    Unsupported: Unsupported Tensor.requires_grad_() call
      Developer debug context: call_method TensorVariable() requires_grad_
    from user code:
      unsloth_zoo/peft_utils.py, line 357, in requires_grad_pre_hook
        first.requires_grad_(True)

Making the hooks opaque to Dynamo fixes that, but it has a second-order
hazard: register_other_hooks() decides which hooks are OURS by matching
...
being recognised and would be re-registered on top of themselves.

torch 2.9 happens to carry the names through torch._dynamo.disable, but that
is an implementation detail of a private module and this has to hold from
torch 2.6 upward, so the wrapper copies them explicitly.
