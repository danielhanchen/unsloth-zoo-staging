...
def _conversion_sibling_info(llama_cpp_dir):
    """Hashable summary of conversion/{__init__,base,qwen}.py mtimes+sizes for
    the patcher cache key. Returns None when conversion/ is absent (monolithic
    upstream layout). Including this in the cache key invalidates the lru_cache
    entry whenever any of the three files changes on disk -- which matters for
    in-place llama.cpp checkouts that get re-pulled."""
...
def _detect_converter_layout(entry_content_bytes, llama_cpp_dir):
    ...
    Detection is structural, not version-based: the entrypoint must contain
    `from conversion import` AND conversion/__init__.py and conversion/base.py
    must exist on disk. Anything else falls back to the legacy single-file
    layout (which the existing patches already handle)."""
    try:
...
def _extract_dict_keys_from_conversion_init(conv_init_path, dict_name):
    """AST-parse conversion/__init__.py and return the set of string-literal
    keys for TEXT_MODEL_MAP / MMPROJ_MODEL_MAP. This is the supported-
    architecture allowlist for the new layout, where ModelBase._model_classes
    is empty at introspection time (per-arch modules register lazily via
    load_all_models()). Returns an empty set on any parse failure -- the
    caller treats that as 'no allowlist available' and skips the arch check."""
    try:
...
def _apply_branding_patch_to_base(conv_base_path):
    """Patch conversion/base.py IN PLACE to inject Unsloth metadata branding
    immediately after `self.metadata = gguf.Metadata.load(...)`. Idempotent
    via a one-line marker. Returns:
        'applied'         -- patch written this call
        'already-applied' -- marker already present, no-op
        'pattern-missing' -- Metadata.load() call not found in expected form
    """
    try:
...
def _qwen_already_handles_expert_aliases(conv_qwen_path):
    """Return True iff conversion/qwen.py already searches both num_local_experts
    and num_experts. When True, the Unsloth Qwen2MoE num_experts patch is a
    no-op against upstream and emitting the legacy warning is misleading.
    Canonical upstream form is
        self.find_hparam(["num_local_experts", "num_experts"])
    at conversion/qwen.py:115 (master)."""
    try:
...
def _download_convert_hf_to_gguf(name = "unsloth_convert_hf_to_gguf"):
    ...
    # The conversion/ sibling info is folded into the cache key too, so an
    # upstream pull of conversion/base.py or conversion/qwen.py invalidates
    # the patcher cache and re-runs the branding patch / arch enumeration.
...
def _download_convert_hf_to_gguf_cached(name, _local_script_info, _conversion_info):
    ...
    # Layout flag: shared between the introspection block (sets it from the
    # downloaded entrypoint) and the patching block (decides whether Patch 2
    # targets the entrypoint or conversion/base.py, and whether Patch 3 should
    # be skipped because upstream already handles the expert alias).
    _layout = "monolith"
    ...
    try:
        ...
        # Upstream llama.cpp was refactored: convert_hf_to_gguf.py is now a thin
        # entrypoint that re-exports ModelBase / ModelType from a `conversion/`
        # package, and per-arch modules register lazily via load_all_models().
        # That means ModelBase._model_classes is empty at introspection time.
        # For the new layout we AST-parse the static TEXT_MODEL_MAP /
        # MMPROJ_MODEL_MAP dicts in conversion/__init__.py instead -- they
        # enumerate every supported arch without importing per-arch modules
        # (which is cheaper, safer, and avoids leaking partially-loaded
        # `conversion` state into the parent process). On the old monolithic
        # layout the existing _model_classes introspection runs unchanged.
    ...
    try:
        ...
        # Old monolithic layout: the `self.metadata = gguf.Metadata.load(...)`
        # call lives in convert_hf_to_gguf.py itself; patch the in-memory
        # `patched_content` bytes that we are about to write next to the
        # entrypoint. New package layout: that call now lives in
        # conversion/base.py:912; patch that file IN PLACE on disk
        # (idempotent via _UNSLOTH_BRANDING_MARKER). On the old layout the
        # patched entrypoint is what runs; on the new layout the entrypoint
        # imports ModelBase from conversion/base.py, so the in-place patch is
        # what takes effect at runtime.
        try:
            if _layout == "package":
        ...
        # The legacy patch rewrites `n_experts = self.hparams["num_experts"]`
        # to read num_local_experts as a fallback. Upstream package layout
        # already does this via `self.find_hparam(["num_local_experts",
        # "num_experts"])` at conversion/qwen.py:115 -- the legacy patch
        # target literal is no longer present and the legacy warning is
        # misleading. Skip silently on new layout when both literals are
        # already present in conversion/qwen.py; otherwise run the existing
        # patch unchanged so monolithic checkouts keep working.
        try:
            _qwen_handled = False
            if _layout == "package":
...
def quantize_gguf(
    ...
    # Q2_K_L is an Unsloth-side preset, not a native llama.cpp ftype. It maps
    # to the q2_k ftype with --output-tensor-type q8_0 and
    # --token-embedding-type q8_0 so the output/embedding tensors retain
    # higher precision than a plain Q2_K quant. Single source of truth for
    # CUDA, MLX/Studio, notebooks, and any future caller.
