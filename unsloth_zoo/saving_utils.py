...
def _hub_repo_weights_without_safetensors(model_name, token = None):
    ...
    for one that is there in a format the merge cannot read. Callers turn False
    into "not found locally or on Hugging Face ... or the name is wrong", which
    sends the user hunting a typo they do not have: `unsloth/bge-m3` is public
    and readable and ships only `pytorch_model.bin`.

    Diagnosis only. Nothing here changes which repos resolve, so a caller that
    never asks is unaffected, and any failure to answer returns None so the
