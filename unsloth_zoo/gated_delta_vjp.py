...
# why: hoist _chunked_forward / _chunked_vjp to module scope so MLX caches the
# compiled graph by Python object identity (defining inside the hot path made
# MLX retrace every call), and pass mask_chunk as a primal so chunk N reads
# the correct absolute mask positions instead of the chunk-0 slice.
@mx.custom_function
def _chunked_forward(q_chunk, k_chunk, v_chunk, g_chunk, beta_chunk, state_in, mask_chunk):
    chunk_T = q_chunk.shape[1]
    has_mask = mask_chunk.shape[-1] > 0
    ys = []
    s = state_in
    for t in range(chunk_T):
        m = mask_chunk[:, t] if has_mask else None
...
def _chunked_vjp(primals, cotangents, outputs):
    ...
    return d_q, d_k, d_v, d_g, d_beta, d_state, mx.zeros_like(mask_chunk)
...
def gated_delta_ops_efficient(
    ...
    # why: encode "no mask" as a zero-width sentinel array so mask_chunk can be
    # a primal of stable rank/dtype across chunks; _chunked_forward checks the
    # last-axis size to decide whether a real mask was provided.
    has_mask = mask is not None
    mask_full = mask if has_mask else mx.zeros((B, 0), dtype=mx.bool_)
    ...
    for c in range(num_chunks):
        ...
        if has_mask:
            mask_c = mask_full[:, t_start:t_end]
        else:
            mask_c = mask_full
        chunk_y, s = _chunked_forward(
            q[:, t_start:t_end],
            k[:, t_start:t_end],
            v[:, t_start:t_end],
            g[:, t_start:t_end],
...
def patch_gated_delta():
    ...
    def patched_gated_delta_update(
        ...
        # _is_training is an explicit hint from the trainer. When unset, only
        # treat T>1 + state-None calls as training; T==1 + state-None is the
        # first inference token and should still go through the kernel.
        if _is_training is None:
            is_training_call = state is None and q.shape[1] > 1
        else:
            is_training_call = bool(_is_training)
        beta = mx.sigmoid(b)
