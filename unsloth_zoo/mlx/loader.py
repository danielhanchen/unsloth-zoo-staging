        ...
        **kwargs,  # Accept and ignore GPU-only kwargs
        ...
        if adapter_cfg_path and os.path.exists(adapter_cfg_path):
            try:
                ...
                # DoRA-unavailable signal from _apply_lora_at_paths) is
                # not swallowed back into the silent standard-load
                # fallback below.
                if isinstance(e, RuntimeError) and "Unsloth MLX:" in str(e):
                    raise
                # If adapter_config.json itself declared a LoRA/DoRA
                # adapter, refuse to fall back to a base model load.
                # Otherwise an ordinary exception (e.g. AttributeError on
                # a malformed lora_parameters block, JSON shape drift,
                # etc.) silently returns a model without adapter weights.
