...
def _enrich_mlx_adapter_config(model, adapter_config):
    ...
    # mlx-lm.load_adapters() reads num_layers + fine_tune_type off the
    # adapter config to wrap the right transformer layers. Trainer.save_model
    # fills these in directly; backfill them here too so the public
    # save_pretrained_merged path produces a loadable artifact.
    ...
    # The inline block below covers the lora_parameters backfill via
    # _infer_mlx_lora_rank and respects explicit caller path filters, so the
    # PR #692 _extract_mlx_lora_parameters fast-path is unnecessary here.
    try:
